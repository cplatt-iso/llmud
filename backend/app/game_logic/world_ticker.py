...
from app.game_logic.mob_ai_ticker import (
    process_aggressive_mobs_task,
    process_roaming_mobs_task,
)
...
async def check_afk_players_task(db: Session):
    ...
    idle_players = [
        player_id
        for player_id, last_seen in list(ws_manager.player_last_seen.items())
...
def db_session_for_world_tick() -> Iterator[Optional[Session]]:
    ...
    if db_session.engine is None:
        logger.critical(
            "World Ticker CRITICAL: Database engine is not initialized. Cannot create session."
        )
...
def register_world_tick_task(
    task_name: str, task_func: Callable[[Session], Awaitable[None]]
):
...
def _initialize_and_register_all_world_tasks():
    ...
    logger.info(
        f"World Ticker: All tasks registered. Active tasks: {list(world_tick_tasks.keys())}"
    )
...
async def world_ticker_loop():
    logger.info(
        f"World Ticker: Loop now running with interval: {WORLD_TICK_INTERVAL_SECONDS}s."
    )
    while True:
        ...
        try:
            with db_session_for_world_tick() as db:
                ...
                if db is None:
                    logger.error(
                        "World Ticker: Skipping tick due to unavailable database session."
                    )
                    await asyncio.sleep(WORLD_TICK_INTERVAL_SECONDS)
                    continue
                ...
                for task_name, task_func in tasks_to_run:
                    try:
                        ...
                        logger.error(
                            f"ERROR in world_tick task '{task_name}': {e}",
                            exc_info=True,
                        )
            ...
            logger.critical(
                f"CRITICAL ERROR in world_ticker_loop's DB session management: {e}",
                exc_info=True,
            )
        ...
        if sleep_duration < 0:
            logger.warning(
                f"World tick processing time ({processing_time:.2f}s) exceeded interval ({WORLD_TICK_INTERVAL_SECONDS}s)."
            )
            sleep_duration = 0
