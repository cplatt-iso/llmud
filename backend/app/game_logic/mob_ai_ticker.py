...
from app.services.room_service import get_player_ids_in_room # <<< We'll use this proper service
...
async def process_roaming_mobs_task(db: Session):
    ...
    mobs_to_check_for_roaming = db.query(models.RoomMobInstance).options(
        ...
        # --- THE FIX IS HERE ---
        models.RoomMobInstance.originating_spawn_definition.has(models.MobSpawnDefinition.roaming_behavior['type'].cast(String) == 'random_adjacent')
    ).all()
    ...
    for mob in mobs_to_check_for_roaming:
        ...
        if player_ids_in_old_room:
            await ws_manager.broadcast_to_players(
                {"type": "game_event", "message": f"{mob_name_html} shuffles off, heading {chosen_direction}."},
        ...
        if player_ids_in_new_room:
            ...
            await ws_manager.broadcast_to_players(
                {"type": "game_event", "message": f"{mob_name_html} arrives from the {opposite_direction}."},
