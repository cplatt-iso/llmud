...
class CharacterClassTemplate(Base):
    ...
    base_stat_modifiers: Mapped[Optional[Dict[str, int]]] = mapped_column(
        JSON, nullable=True
    )
    starting_health_bonus: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )
    starting_mana_bonus: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    skill_tree_definition: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True
    )
    starting_equipment_refs: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True, default=lambda: []
    )
    playstyle_tags: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True, default=lambda: []
    )

    # <<< NEW FIELD >>>
    stat_gains_per_level: Mapped[Optional[Dict[str, Union[int, float]]]] = (
        mapped_column(
            JSON,
            nullable=True,
