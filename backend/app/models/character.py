...
from sqlalchemy import (  # Ensure Boolean is imported
    JSON,
    Boolean,
    Column,
    Enum,
...
class Character(Base):
    ...
    class_name: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Adventurer"
    )  # Keep as stored field
    ...
    god_level: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="0 for mortals, 1-10 for gods"
    )
    titles: Mapped[Optional[List[str]]] = mapped_column(
        JSON,
        nullable=True,
        default=lambda: [],
        comment="e.g., ['The Godslayer', 'Cheesewheel Enthusiast']",
    )
    current_room_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("rooms.id"), nullable=False, index=True
    )
    is_brief_mode: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # --- Class Template Link ---
    character_class_template_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("character_class_templates.id"), nullable=True
    )
    ...
    class_template_ref: Mapped[Optional["CharacterClassTemplate"]] = relationship(
        foreign_keys=[character_class_template_id]
    )
    ...
    # --- Skills & Traits ---
    learned_skills: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True, default=lambda: []
    )
    learned_traits: Mapped[Optional[List[str]]] = mapped_column(
        JSON, nullable=True, default=lambda: []
    )

    # --- Basic Combat Stats (Placeholders until derived from gear/attributes) ---
    base_ac: Mapped[int] = mapped_column(
        Integer, default=10, nullable=False, comment="Base Armor Class"
    )
    base_attack_bonus: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Base to-hit bonus"
    )
    base_damage_dice: Mapped[str] = mapped_column(
        String(20), default="1d4", nullable=False, comment="e.g., 1d6"
    )
    base_damage_bonus: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="Base flat damage bonus"
    )
    ...
    trait_points: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", nullable=False
    )

    autoloot_enabled: Mapped[bool] = mapped_column(
    ...
    def get_attribute_modifier(self, attribute_name: str) -> int:
        ...
        score = getattr(
            self, attribute_name, 10
        )  # Default to 10 if attribute somehow not found
        return (score - 10) // 2
    ...
    def get_equipped_items_by_slot(self, slot_key: str) -> List["CharacterInventoryItem"]:  # type: ignore
        ...
        return [
            ...
            if inv_item.equipped and inv_item.equipped_slot == slot_key
        ]
    ...
    def calculate_combat_stats(self) -> Dict[str, Any]:
        ...
        equipped_weapon_item: Optional[models.Item] = (
            None  # Store the Item model directly
        )
        ...
        if not hasattr(self, "inventory_items") or self.inventory_items is None:
            ...
            print(
                f"Warning: Character {self.name} has no inventory_items attribute or it's None during stat calculation."
            )
        else:
            for inv_item in self.inventory_items:
                ...
                if (
                    item_model.item_type == "armor"
                ):  # This covers torso, head, feet, etc. AND shields if type is "armor"
                    ...
                    if "max_dex_bonus_to_ac" in item_props:
                        ...
                        if (
                            max_dex_bonus_cap_from_armor is None
                            or cap < max_dex_bonus_cap_from_armor
                        ):
                            max_dex_bonus_cap_from_armor = cap

                # Specifically identify the main hand weapon
                if (
                    inv_item.equipped_slot == "main_hand"
                    and item_model.item_type == "weapon"
                ):
        ...
        if equipped_weapon_item:
            ...
            stats["damage_dice"] = weapon_props.get(
                "damage", "1d2"
            )  # Default to unarmed if weapon has no damage prop
            ...
            stats["attack_bonus"] = attack_attribute_modifier + weapon_props.get(
                "attack_bonus", 0
            )
            stats["damage_bonus"] = attack_attribute_modifier + weapon_props.get(
                "damage_bonus", 0
            )
        else:
