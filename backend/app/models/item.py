...
from sqlalchemy import (  # Keep Column for __tablename__ etc.
    JSON,
    Boolean,
    Column,
    Float,
...
class Item(Base):
    ...
    item_type: Mapped[str] = mapped_column(
        ...
        index=True,
        nullable=False,
        comment="e.g., weapon, armor, potion, junk",
    )
    ...
    rarity: Mapped[str] = mapped_column(
        String(50), nullable=False, default="common", index=True
    )
    slot: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="e.g., head, torso, main_hand, off_hand, ring, consumable, utility",
    )

    properties: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON,
        nullable=True,
        comment="e.g., {'damage': '1d6', 'armor_class': 5, 'modifier': {'strength': 1}}",
    )

    weight: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    value: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="Monetary value for shops"
    )
    stackable: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    max_stack_size: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        default=1,
        comment="Max items in a stack if stackable, 1 if not.",
    )
