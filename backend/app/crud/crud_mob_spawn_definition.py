...
from ..crud import crud_room, crud_mob
...
def _load_spawn_definitions_from_json() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Spawn definition file not found: {filepath}. No spawns will be seeded.")
        return []
    except json.JSONDecodeError as e:
        logger.error(f"Could not decode JSON from {filepath}: {e}. Spawn seeding aborted.")
        return []
...
def seed_initial_mob_spawn_definitions(db: Session):
    ...
    for def_data in spawn_definitions_data:
        ...
        if not def_name:
            logger.warning(f"Skipping spawn definition due to missing 'definition_name': {def_data}")
            skipped_count += 1
            continue
        ...
        if "room_coords" in def_data:
            ...
            room = crud_room.get_room_by_coords(db, x=coords.get('x'), y=coords.get('y'), z=coords.get('z'))
            if not room:
                logger.error(f"For spawn '{def_name}', could not find room at coords {coords}. Skipping.")
                skipped_count += 1
                continue
        else:
            logger.error(f"Spawn definition '{def_name}' is missing 'room_coords'. Skipping.")
            skipped_count += 1
            continue
        
        mob_template = None
        if "mob_template_name" in def_data:
            mob_template = crud_mob.get_mob_template_by_name(db, name=def_data["mob_template_name"])
            if not mob_template:
                logger.error(f"For spawn '{def_name}', could not find mob template named '{def_data['mob_template_name']}'. Skipping.")
                skipped_count += 1
                continue
        else:
            logger.error(f"Spawn definition '{def_name}' is missing 'mob_template_name'. Skipping.")
            skipped_count += 1
            continue
        ...
        try:
            ...
            pydantic_data.pop('room_coords', None) # Remove keys not in the Pydantic model
            pydantic_data.pop('mob_template_name', None)

            existing_def = get_mob_spawn_definition_by_name(db, definition_name=def_name)
            ...
            if not existing_def:
                ...
                original_dump = schemas.MobSpawnDefinition.from_orm(existing_def).model_dump()
                
                update_mob_spawn_definition(db, db_definition=existing_def, definition_in=spawn_update_schema)

                # Check if it was actually modified to log correctly
                db.flush() # Flush to see changes in the session
                updated_dump = schemas.MobSpawnDefinition.from_orm(existing_def).model_dump()
            ...
            logger.error(f"Error processing spawn definition '{def_name}': {e}", exc_info=True)
            skipped_count += 1
            db.rollback() # Rollback this specific entry's changes

    if seeded_count > 0 or updated_count > 0:
        logger.info(f"Committing {seeded_count} new and {updated_count} updated mob spawn definitions.")
        db.commit()
    else:
