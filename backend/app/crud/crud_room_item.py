...
def get_room_item_instance(
    db: Session, room_item_instance_id: uuid.UUID
) -> Optional[models.RoomItemInstance]:
    return (
        db.query(models.RoomItemInstance)
        .options(joinedload(models.RoomItemInstance.item))  # Eager load item details
        .filter(models.RoomItemInstance.id == room_item_instance_id)
        .first()
    )
...
def get_items_in_room(db: Session, room_id: uuid.UUID) -> List[models.RoomItemInstance]:
    ...
    return (
        db.query(models.RoomItemInstance)
        .options(joinedload(models.RoomItemInstance.item))
        .filter(models.RoomItemInstance.room_id == room_id)
        .all()
    )
...
def add_item_to_room(
    ...
    *,
    ...
    properties_override: Optional[
        Dict
    ] = None,  # Note: properties_override is for the RoomItemInstance, not Item template
    ...
    if item_template.stackable:
        ...
        if existing_stack:
            max_stack = (
                item_template.max_stack_size
                if item_template.max_stack_size is not None
                else float("inf")
            )
            ...
            if quantity <= max_stack:
                ...
                return (
                    None,
                    f"Cannot drop stack of {quantity}; exceeds max stack size of {item_template.max_stack_size} for {item_template.name}.",
                )
...
def remove_item_from_room(
    db: Session, *, room_item_instance_id: uuid.UUID, quantity_to_remove: int = 1
) -> Tuple[Optional[models.RoomItemInstance], str]:
    """
    ...
    if instance.quantity > quantity_to_remove:
        ...
        return (
            instance,
            f"Staged pickup of {quantity_to_remove} x {original_item_name}. {instance.quantity} remaining.",
        )
    else:
        ...
        return (
            None,
            f"Staged pickup (deletion) of all {removed_qty} x {original_item_name} from ground.",
        )
