...
def search_items_by_name(db: Session, name_part: str) -> List[models.Item]:
    ...
    return (
        ...
        .filter(func.lower(models.Item.name).ilike(f"%{name_part.lower()}%"))
        .all()
    )
...
def get_item_by_item_tag(db: Session, item_tag: str) -> Optional[models.Item]:
    return (
        db.query(models.Item)
        .filter(models.Item.properties["item_tag"].astext == item_tag)
        .first()
    )
...
def seed_initial_items(db: Session):
    ...
    if not item_definitions:
        logger.warning(
            "No item definitions found or error loading items.json. Aborting item seeding."
        )
        return
    ...
    for item_data in item_definitions:
        ...
        try:
            if existing_item:
                ...
                changed = False
                for field, value in item_update_schema.model_dump(
                    exclude_unset=True
                ).items():
                ...
                if changed:
                    ...
                    skipped_count += (
                        1  # Not really skipped, but not "newly seeded" or "updated"
                    )
            else:
            ...
            logger.error(
                f"Pydantic validation or DB operation failed for item '{item_name}': {e_pydantic}. Data: {item_data}"
            )
            skipped_count += 1
            continue
    ...
    if seeded_count > 0 or updated_count > 0:
        try:
            logger.info(
                f"Committing {seeded_count} new items and {updated_count} updated items."
            )
            db.commit()
        ...
        logger.info(
            "No new items to seed or items to update. No commit needed for items."
        )

    logger.info(
        f"Item seeding complete. New: {seeded_count}, Updated: {updated_count}, Unchanged/Skipped: {skipped_count}"
    )
