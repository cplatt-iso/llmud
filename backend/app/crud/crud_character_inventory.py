...
from .. import (  # models.Item, models.Character, models.CharacterInventoryItem
    models,
    schemas,
)
...
def get_inventory_item_entry_by_id(
    db: Session, inventory_item_id: uuid.UUID
) -> Optional[models.CharacterInventoryItem]:
    return (
        db.query(models.CharacterInventoryItem)
        .options(joinedload(models.CharacterInventoryItem.item))
        .filter(models.CharacterInventoryItem.id == inventory_item_id)
        .first()
    )


def get_character_inventory(
    db: Session, character_id: uuid.UUID
) -> List[models.CharacterInventoryItem]:
    return (
        db.query(models.CharacterInventoryItem)
        .options(joinedload(models.CharacterInventoryItem.item))
        .filter(models.CharacterInventoryItem.character_id == character_id)
        .all()
    )


def character_has_item_with_tag(
    db: Session, character_id: uuid.UUID, item_tag: str
) -> bool:
    """Checks if a character has an item with a specific 'item_tag' in its properties."""
    count = (
        db.query(models.CharacterInventoryItem.id)
        .join(models.Item, models.CharacterInventoryItem.item_id == models.Item.id)
        .filter(
    ...
    logger.debug(
        f"Checked for item_tag '{item_tag}' for char {character_id}. Found: {count > 0}"
    )  # Good to add a log here
    return count > 0
...
def add_item_to_character_inventory(
    db: Session,
    *,
    character_obj: models.Character,
    item_id: uuid.UUID,
    quantity: int = 1,
    ...
    if not character_obj.id:
        logger.warning(
            "Character object does not have an ID yet. Flushing session to assign ID before adding inventory."
        )
        try:
            db.flush([character_obj])  # Flush only this object to get an ID if it's new
            if not character_obj.id:  # Still no ID after flush
                logger.error(
                    "Failed to obtain character ID even after flush. Cannot add item to inventory."
                )
    ...
    if item_template.stackable:
        ...
        if existing_unequipped_stack:
            ...
            if (
                effective_max_stack_template is None
            ):  # No defined limit on the item template
                ...
                space_in_existing_stack = (
                    effective_max_stack_template - existing_unequipped_stack.quantity
                )
            ...
            can_add_to_this_stack = min(
                remaining_quantity_to_add, space_in_existing_stack
            )
            ...
            if can_add_to_this_stack > 0:
                ...
                messages.append(
                    f"Added {can_add_to_this_stack} to existing stack of {item_template.name}."
                )
        ...
        while remaining_quantity_to_add > 0:
            ...
            if effective_max_stack_template is None:  # No limit for new stacks either
                current_add_amount = (
                    remaining_quantity_to_add  # Create one new stack with all remaining
                )
            elif remaining_quantity_to_add > effective_max_stack_template:
                current_add_amount = (
                    effective_max_stack_template  # Fill one new stack to max
                )
            else:
                current_add_amount = (
                    remaining_quantity_to_add  # New stack with the rest
                )
            ...
            messages.append(
                f"Created new stack of {item_template.name} with {current_add_amount}."
            )
        ...
        if total_added_successfully > 0:
            messages.append(
                f"Added {total_added_successfully} individual {item_template.name}(s)."
            )
    ...
    if total_added_successfully == quantity and quantity > 0:
        ...
        if len(messages) == 1:
            final_message = (
                messages[0].replace("Staged ", "").capitalize()
            )  # Cleaner message
        elif len(messages) > 1:
...
def remove_item_from_character_inventory(
    ...
    if entry.equipped:
        ...
        return (
            None,
            f"Cannot remove '{item_name}'; it is currently equipped. Unequip it first.",
        )
    ...
    if entry.quantity > quantity_to_remove:
        ...
        return (
            entry,
            f"Staged removal of {quantity_to_remove} x {original_item_name}. {entry.quantity} remaining.",
        )
...
def equip_item_from_inventory(
    ...
    *,
    ...
    logger.debug(
        f"[CRUD_EQUIP] Attempting to equip item_id: {inventory_item_id} for char: {character_obj.name} to slot: {target_slot}"
    )
    char_inv_entry = get_inventory_item_entry_by_id(db, inventory_item_id)

    if not char_inv_entry:
        logger.warning(
            f"[CRUD_EQUIP_FAIL] Item instance {inventory_item_id} not found."
        )
        return None, "Item instance not found in your inventory records."
    if char_inv_entry.character_id != character_obj.id:
        logger.warning(
            f"[CRUD_EQUIP_FAIL] Ownership mismatch: Item {inventory_item_id} (owner: {char_inv_entry.character_id}) vs Char {character_obj.id}"
        )
        return None, "This item does not belong to you."

    item_name_for_log = (
        char_inv_entry.item.name if char_inv_entry.item else "UnknownItem"
    )
    if char_inv_entry.equipped:
        logger.info(
            f"[CRUD_EQUIP_FAIL] Item '{item_name_for_log}' ({inventory_item_id}) already equipped in {char_inv_entry.equipped_slot}."
        )
        return (
            char_inv_entry,
            f"{item_name_for_log} is already equipped in {char_inv_entry.equipped_slot}.",
        )

    item_template = char_inv_entry.item
    if not item_template:
        logger.error(
            f"[CRUD_EQUIP_FAIL] Inventory item {char_inv_entry.id} is missing its item_template relationship."
        )
        return None, "Item template data missing for this inventory item."

    if not item_template.slot or item_template.slot in [
        "consumable",
        "inventory",
        "junk",
        "key",
        ...
        logger.warning(
            f"[CRUD_EQUIP_FAIL] Item '{item_template.name}' (type: {item_template.item_type}, defined slot: {item_template.slot}) is not equippable."
        )
    ...
    if not final_target_slot:
        if item_defined_slot_type in EQUIPMENT_SLOTS:
            ...
            logger.debug(
                f"[CRUD_EQUIP] Auto-determined slot for '{item_template.name}' to be '{final_target_slot}' based on item.slot '{item_defined_slot_type}'."
            )
        elif item_defined_slot_type == "ring":
            logger.warning(
                f"[CRUD_EQUIP_FAIL] Specific finger slot not provided for ring '{item_template.name}'."
            )
            return (
                None,
                f"Please specify which finger slot to equip {item_template.name} (e.g., equip {item_template.name} finger_1).",
            )
        else:
            logger.warning(
                f"[CRUD_EQUIP_FAIL] Cannot auto-determine slot for '{item_template.name}' (type: {item_defined_slot_type}). User must specify."
            )
            return (
                None,
                f"Cannot automatically determine slot for {item_template.name} (defined slot type: {item_defined_slot_type}). Please specify a target slot.",
            )

    if final_target_slot not in EQUIPMENT_SLOTS:
        logger.warning(
            f"[CRUD_EQUIP_FAIL] Invalid target_slot '{final_target_slot}' specified by user or logic."
        )
        return (
            None,
            f"Invalid equipment slot: '{final_target_slot}'. Valid slots are: {', '.join(EQUIPMENT_SLOTS.keys())}.",
        )
    ...
    if item_defined_slot_type == "ring" and not final_target_slot.startswith("finger"):
        return (
            None,
            f"{item_template.name} (a ring) cannot be equipped to {EQUIPMENT_SLOTS.get(final_target_slot, final_target_slot)}.",
        )
    if item_defined_slot_type == "main_hand" and final_target_slot not in [
        "main_hand",
        "off_hand",
    ]:
        return (
            None,
            f"{item_template.name} (main_hand weapon) cannot be equipped to {EQUIPMENT_SLOTS.get(final_target_slot, final_target_slot)}.",
        )
    if (
        item_defined_slot_type == "off_hand" and final_target_slot != "off_hand"
    ):  # Shields, etc.
        return (
            None,
            f"{item_template.name} (off_hand item) cannot be equipped to {EQUIPMENT_SLOTS.get(final_target_slot, final_target_slot)}.",
        )
    # Ensure a general item (e.g. item.slot = "torso") is being equipped to the correct slot
    if (
        item_defined_slot_type != "ring"
        and item_defined_slot_type not in ["main_hand", "off_hand"]
        and item_defined_slot_type != final_target_slot
    ):
        return (
            None,
            f"{item_template.name} (for {item_defined_slot_type}) cannot be equipped to {EQUIPMENT_SLOTS.get(final_target_slot, final_target_slot)}.",
        )
    ...
    for item_to_unequip_orm in current_character_inventory_snapshot:
        if item_to_unequip_orm.id == char_inv_entry.id:
            continue  # Don't try to unequip the item we are trying to equip
        if (
            item_to_unequip_orm.equipped
            and item_to_unequip_orm.equipped_slot == final_target_slot
        ):
            logger.info(
                f"[CRUD_EQUIP] Unequipping '{item_to_unequip_orm.item.name if item_to_unequip_orm.item else 'item'}' from slot '{final_target_slot}' to make space."
            )
    ...
    if item_template.properties and item_template.properties.get("two_handed", False):
        ...
        if final_target_slot == "main_hand":
            other_hand_slot = "off_hand"
        elif final_target_slot == "off_hand":
            other_hand_slot = "main_hand"  # Equipping two-hander in off_hand implies main is also used

        if other_hand_slot:
            for other_hand_item_orm in current_character_inventory_snapshot:
                if other_hand_item_orm.id == char_inv_entry.id:
                    continue
                if (
                    other_hand_item_orm.equipped
                    and other_hand_item_orm.equipped_slot == other_hand_slot
                ):
                    logger.info(
                        f"[CRUD_EQUIP] Unequipping '{other_hand_item_orm.item.name if other_hand_item_orm.item else 'item'}' from '{other_hand_slot}' for two-handed '{item_template.name}'."
                    )
    ...
    if item_template.stackable and char_inv_entry.quantity > 1:
        ...
        logger.debug(
            f"Split stack for equipping: {remainder_quantity} of {item_template.name} remains in backpack."
        )
        # Modify the current entry to be quantity 1 and equipped
        char_inv_entry.quantity = 1

    logger.info(
        f"[CRUD_EQUIP] Setting item '{item_template.name}' (ID: {char_inv_entry.id}): equipped=True, equipped_slot='{final_target_slot}'"
    )
    ...
    return (
        char_inv_entry,
        f"Staged equipping of {item_template.name} to {EQUIPMENT_SLOTS.get(final_target_slot, final_target_slot)}.",
    )
...
def unequip_item_to_inventory(
    ...
    *,
    ...
    slot_to_unequip: Optional[str] = None,
) -> Tuple[Optional[models.CharacterInventoryItem], str]:
    ...
    if inventory_item_id:
        ...
        for item_instance in current_character_inventory_snapshot:
            if item_instance.id == inventory_item_id:
                if item_instance.character_id == character_obj.id:
                    ...
                    logger.warning(
                        f"Attempt to unequip item {inventory_item_id} by char {character_obj.name} but item belongs to char_id {item_instance.character_id}"
                    )
        ...
        for item_instance in current_character_inventory_snapshot:
            if (
                ...
                and item_instance.equipped_slot == slot_to_unequip
            ):
                char_inv_entry_to_unequip = item_instance
                break
    else:
    ...
    if not char_inv_entry_to_unequip.item:
        logger.error(
            f"Equipped item {char_inv_entry_to_unequip.id} is missing its item_template data."
        )
        return None, "Item template data missing for this equipped item."

    if (
        not char_inv_entry_to_unequip.equipped
        or not char_inv_entry_to_unequip.equipped_slot
    ):
        return (
            char_inv_entry_to_unequip,
            f"{char_inv_entry_to_unequip.item.name} is not currently equipped.",
        )

    item_name_unequipped = char_inv_entry_to_unequip.item.name
    slot_display_name = EQUIPMENT_SLOTS.get(
        char_inv_entry_to_unequip.equipped_slot, char_inv_entry_to_unequip.equipped_slot
    )
    ...
    if char_inv_entry_to_unequip.item.stackable:
        ...
        # Let's try to merge:
        potential_merge_stack = (
            db.query(models.CharacterInventoryItem)
            .filter(
                models.CharacterInventoryItem.character_id == character_obj.id,
                models.CharacterInventoryItem.item_id
                == char_inv_entry_to_unequip.item_id,
                models.CharacterInventoryItem.equipped == False,
                models.CharacterInventoryItem.id
                != char_inv_entry_to_unequip.id,  # Exclude the item we just unequipped itself
            )
            .first()
        )
        ...
        if potential_merge_stack:
            max_stack = (
                char_inv_entry_to_unequip.item.max_stack_size
                if char_inv_entry_to_unequip.item.max_stack_size is not None
                else float("inf")
            )
            if (
                ...
                logger.debug(
                    f"Merging unequipped {item_name_unequipped} (qty {char_inv_entry_to_unequip.quantity}) into existing stack (qty {potential_merge_stack.quantity})"
                )
                potential_merge_stack.quantity += char_inv_entry_to_unequip.quantity
                db.add(potential_merge_stack)
                db.delete(
                    char_inv_entry_to_unequip
                )  # Delete the now-merged original entry
    ...
    return (
        char_inv_entry_to_unequip,
        f"Staged unequipping of {item_name_unequipped} from {slot_display_name}.",
    )
