...
from typing import Any, Dict, List, Optional, Tuple  # Added Any for seed data
...
) -> List[Dict[str, Any]]:  # Made generic
...
def get_mob_template(
    db: Session, mob_template_id: uuid.UUID
) -> Optional[models.MobTemplate]:
    return (
        db.query(models.MobTemplate)
        .filter(models.MobTemplate.id == mob_template_id)
        .first()
    )
...
) -> List[models.MobTemplate]:
    return db.query(models.MobTemplate).offset(skip).limit(limit).all()


def create_mob_template(
    db: Session, *, template_in: schemas.MobTemplateCreate
) -> models.MobTemplate:
...
def update_mob_template(
    db: Session,
    *,
    db_template: models.MobTemplate,
    template_in: schemas.MobTemplateUpdate,
) -> models.MobTemplate:
...
def get_room_mob_instance(
    db: Session, room_mob_instance_id: uuid.UUID
) -> Optional[models.RoomMobInstance]:
    return (
        db.query(models.RoomMobInstance)
        .options(joinedload(models.RoomMobInstance.mob_template))
        .filter(models.RoomMobInstance.id == room_mob_instance_id)
        .first()
    )
...
def get_mobs_in_room(db: Session, room_id: uuid.UUID) -> List[models.RoomMobInstance]:
    return (
        db.query(models.RoomMobInstance)
        .options(joinedload(models.RoomMobInstance.mob_template))
        .filter(models.RoomMobInstance.room_id == room_id)
        .all()
    )
...
def spawn_mob_in_room(
    ...
    *,
    ...
    if not template:
        ...
        return None
    room = (
        db.query(models.Room).filter(models.Room.id == room_id).first()
    )  # Consider using crud.crud_room.get_room_by_id
    if not room:
    ...
    logger.info(
        f"Staged spawn of mob '{template.name}' (Template ID: {template.id}) in room '{room.name}' (Room ID: {room_id}). Instance ID will be assigned on commit."
    )
...
def despawn_mob_from_room(db: Session, room_mob_instance_id: uuid.UUID) -> bool:
    ...
    if instance:
        ...
        mob_name_for_log = (
            instance.mob_template.name if instance.mob_template else "Unknown Mob"
        )
        logger.info(
            f"Despawning mob '{mob_name_for_log}' (Instance ID: {room_mob_instance_id})."
        )
        ...
        if spawn_def_id_to_update:
            ...
            logger.debug(
                f"Flagged immediate re-check for spawn definition {spawn_def_id_to_update} due to mob despawn."
            )

        # db.commit() # Caller of despawn (e.g. combat processor) should handle commit
        return True
    logger.warning(
        f"despawn_mob_from_room: Mob instance ID {room_mob_instance_id} not found."
    )
    return False
...
def update_mob_instance_health(
    ...
    if instance and instance.mob_template:
        ...
        instance.current_health = max(
            0, min(instance.current_health, instance.mob_template.base_health)
        )
        ...
        instance
    ):  # Mob instance exists, but template somehow missing (should not happen with joinedload)
        logger.warning(
            f"update_mob_instance_health: Mob instance {room_mob_instance_id} missing mob_template. Health update might be unreliable."
        )
        ...
        logger.warning(
            f"update_mob_instance_health: Mob instance ID {room_mob_instance_id} not found."
        )
    return None
...
def seed_initial_mob_templates(db: Session):
    ...
    mob_template_definitions = _load_seed_data_generic(
        "mob_templates.json", "Mob template"
    )

    if not mob_template_definitions:
        logger.warning(
            "No mob template definitions found or error loading mob_templates.json. Aborting mob template seeding."
        )
        return
    ...
    for template_data in mob_template_definitions:
        ...
        if not template_name:
            logger.warning(
                f"Skipping mob template entry due to missing name: {template_data}"
            )
            skipped_count += 1
            continue
        ...
        try:
            if existing_template:
                ...
                updated_template = update_mob_template(
                    db,
                ...
                original_dump = schemas.MobTemplate.from_orm(
                    existing_template
                ).model_dump(
                    exclude={"id"}
                )  # Exclude id for comparison
                updated_dump_from_data = schemas.MobTemplateCreate(
                    **template_data
                ).model_dump()  # Create a full model from data
                ...
                if (
                    is_actually_changed
                ):  # If update_mob_template would have made changes
            ...
            logger.error(
                f"Validation or DB operation failed for mob template '{template_name}': {e_pydantic_or_db}. Data: {template_data}",
                exc_info=True,
            )
            skipped_count += 1
    ...
    if seeded_count > 0 or updated_count > 0:
        try:
            logger.info(
                f"Committing {seeded_count} new and {updated_count} updated mob templates."
            )
            db.commit()
            logger.info("Mob template seeding commit successful.")
        except Exception as e_commit:
            logger.error(
                f"Error committing mob template seeds: {e_commit}. Rolling back.",
                exc_info=True,
            )
            db.rollback()
    else:
        logger.info(
            "No new mob templates to seed or templates to update. No commit needed for mob templates."
        )

    logger.info(
        f"Mob template seeding complete. New: {seeded_count}, Updated: {updated_count}, Unchanged/Skipped: {skipped_count}"
    )
