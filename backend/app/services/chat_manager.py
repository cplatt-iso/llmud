...
class ChatManager:
            ...
            with open(filepath, 'r') as f:
                ...
                for i, channel_data in enumerate(channel_data_list):
                    try:
                        ...
                        logger.error(f"ChatManager: Validation error for channel #{i+1} in chat_channels.json: {e}")
                        continue # Skip the malformed channel

                logger.info(f"ChatManager: Loaded {len(self.channels)} valid channels.")
        except Exception as e:
            logger.error(f"ChatManager: FATAL error loading or parsing chat_channels.json: {e}", exc_info=True)
    ...
    def unsubscribe_player(self, player_id: uuid.UUID, channel_tag: str):
        if channel_tag in self.subscriptions:
            ...
            logger.debug(f"Player {player_id} unsubscribed from channel '{channel_tag}'.")
