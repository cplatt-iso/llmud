...
from .... import crud, schemas  # Import the schemas package
...
@router.get(
    "/{x}/{y}/{z}", response_model=schemas.RoomInDB
)  # Ensure this is schemas.RoomInDB
async def read_room_details_by_coords(
    ...
    if room_pydantic_schema is None:
        ...
        raise HTTPException(
            status_code=404,
            detail="Room not found. You've wandered off the edge of my persistent patience.",
        )
...
async def read_room_details_by_uuid_str(
    room_id_str: str, db: Session = Depends(get_db)
):
    try:
        room_uuid = uuid.UUID(room_id_str)
    except ValueError:
        raise HTTPException(
            status_code=400, detail=f"Invalid room UUID format: {room_id_str}"
        )

    room_orm = crud.crud_room.get_room_by_id(db, room_id=room_uuid)
    if not room_orm:
        raise HTTPException(
            status_code=404, detail=f"Room with UUID {room_uuid} not found."
        )
