...
class SkillTemplateBase(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=3,
        max_length=100,
        description="Player-facing name, e.g., 'Power Attack'",
    )
    description: Optional[str] = None
    skill_type: str = Field(
        ..., description="e.g., 'COMBAT_ACTIVE', 'PASSIVE', 'UTILITY_OOC'"
    )
    target_type: str = Field(
        default="NONE", description="e.g., 'SELF', 'ENEMY_MOB', 'NONE'"
    )
...
class SkillTemplateUpdate(BaseModel):  # For partial updates
    skill_id_tag: Optional[str] = Field(
