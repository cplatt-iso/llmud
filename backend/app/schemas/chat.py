...
class ChatChannel(BaseModel):
    ...
    """
    channel_id_tag: str = Field(..., description="Unique internal identifier, e.g., 'ooc'.")
    display_name: str = Field(..., description="Player-facing name, e.g., 'OOC'.")
    command_aliases: List[str] = Field(..., description="List of commands that trigger this channel.")
    access_type: Literal["public", "permissioned", "private"]
    required_permission: Optional[str] = Field(None, description="The attribute name on the Player model to check for permission, e.g., 'is_sysop'.")
