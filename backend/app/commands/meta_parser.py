...
from app import crud, models, schemas  # <<< ADDED models
...
async def handle_autoloot(context: CommandContext) -> schemas.CommandResponse:
    ...
    character_orm = crud.crud_character.get_character(
        context.db, character_id=character.id
    )
...
async def handle_help(context: CommandContext) -> schemas.CommandResponse:
    categories = {
        ...
        "Inventory & Items": [
            ...
            (
                "equip (eq) <item_ref> [slot]",
                "Equips an item (e.g., 'eq Sword main_hand').",
            ),
    ...
    help_message_lines = [
        "<span class='inv-section-header'>--- Available Commands ---</span>"
    ]
...
async def handle_skills(context: CommandContext) -> schemas.CommandResponse:
    ...
    for skill_tag in char_skills:
        skill_template = crud.crud_skill.get_skill_template_by_tag(
            context.db, skill_id_tag=skill_tag
        )
        if skill_template:
            ...
            if skill_template.description:
                ...
                if len(skill_template.name) >= 23:
                    desc_indent = "\n    "  # Newline if name is too long
                line += f"{desc_indent}- {skill_template.description}"
            message_lines.append(line)
        else:
            message_lines.append(
                f"  Unknown skill ID: {skill_tag} (This is probably a bug, you poor sod.)"
            )
...
async def handle_traits(context: CommandContext) -> schemas.CommandResponse:
    ...
    if not char_traits:
        message_to_player = (
            "You possess no noteworthy traits. You are remarkably unremarkable."
        )
        return schemas.CommandResponse(
            room_data=context.current_room_schema, message_to_player=message_to_player
        )
    ...
    for trait_tag in char_traits:
        trait_template = crud.crud_trait.get_trait_template_by_tag(
            context.db, trait_id_tag=trait_tag
        )
        if trait_template:
            ...
            if trait_template.description:
                ...
                if len(trait_template.name) >= 23:
                    desc_indent = "\n    "
                line += f"{desc_indent}- {trait_template.description}"
            message_lines.append(line)
        else:
            message_lines.append(
                f"  Unknown trait ID: {trait_tag} (The devs are slacking again.)"
            )
