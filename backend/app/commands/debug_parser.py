...
from app.websocket_manager import (  # Ensure connection_manager is imported
    connection_manager,
)
...
async def handle_giveme(context: CommandContext) -> schemas.CommandResponse:
    ...
    if context.args:
        ...
        item_template = crud.crud_item.get_item_by_name(
            context.db, name=item_name_to_give
        )
        if item_template:
            _, add_message = (
            ...
            context.active_character = crud.crud_character.get_character(
...
async def handle_spawnmob(context: CommandContext) -> schemas.CommandResponse:
    ...
    if not mob_template_name:  # Handle if args were just spaces
        return schemas.CommandResponse(
            ...
            message_to_player="Debug: Please specify a mob template name.",
        )

    target_room_id = context.current_room_orm.id  # Default to current room

    mob_template = crud.crud_mob.get_mob_template_by_name(
        context.db, name=mob_template_name
    )
    if not mob_template:
    ...
    spawned_mob = crud.crud_mob.spawn_mob_in_room(
        db=context.db, room_id=target_room_id, mob_template_id=mob_template.id
    )
    ...
    if spawned_mob:
        ...
        mob_display_name = (
            spawned_mob.mob_template.name
            if spawned_mob.mob_template
            else mob_template_name
        )
...
async def handle_set_hp(context: CommandContext) -> schemas.CommandResponse:
    if not context.args:
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Usage: set_hp <value>",
        )
    try:
        ...
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Invalid HP value. Must be an integer.",
        )
    ...
    return schemas.CommandResponse(
        room_data=context.current_room_schema,
        message_to_player=f"HP set to {character.current_health}/{character.max_health}.",
    )
...
async def handle_mod_xp(context: CommandContext) -> schemas.CommandResponse:
    if not context.args:
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Usage: mod_xp <amount>",
        )
    try:
        ...
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Invalid XP amount. Must be an integer.",
        )

    updated_char, messages = crud.crud_character.add_experience(
        context.db, context.active_character.id, amount
    )

    if not updated_char:
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Error modifying XP.",
        )
    ...
    logger.info(
        f"Debug command mod_xp used for char {updated_char.name}. Broadcasted who_list_updated."
    )

    full_message = "\n".join(messages)
    return schemas.CommandResponse(
        room_data=context.current_room_schema, message_to_player=full_message
    )
...
async def handle_set_level(context: CommandContext) -> schemas.CommandResponse:
    if not context.args:
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Usage: set_level <level>",
        )
    try:
        ...
        if target_level < 1:
            return schemas.CommandResponse(
                room_data=context.current_room_schema,
                message_to_player="Target level must be 1 or greater.",
            )
    except ValueError:
        return schemas.CommandResponse(
            room_data=context.current_room_schema,
            message_to_player="Invalid level. Must be an integer.",
        )

    character = context.active_character
    messages = [
        f"Attempting to set level to {target_level}... Current XP: {character.experience_points}, Level: {character.level}"
    ]
    ...
    while character.level < target_level and safety_counter < max_iterations:
        ...
        if xp_to_next_level == float("inf"):
            messages.append(
                f"Max defined level ({character.level}) reached. Cannot set level to {target_level} by leveling up further."
            )
            break
        ...
        character.experience_points = int(
            crud.crud_character.get_xp_for_level(character.level)
        )
        context.db.add(character)
        safety_counter += 1
    ...
    if safety_counter >= max_iterations:
        messages.append(
            "Warning: Max iterations reached in set_level. Level may not be correctly set."
        )
    ...
    await connection_manager.broadcast({"type": "who_list_updated"})  # Broadcast update
    logger.info(
        f"Debug command set_level used for char {character.name}. Broadcasted who_list_updated."
    )

    messages.append(
        f"Character is now Level {character.level} with {character.experience_points} XP."
    )
    full_message = "\n".join(messages)
    return schemas.CommandResponse(
        room_data=context.current_room_schema, message_to_player=full_message
...
async def handle_set_money(context: CommandContext) -> schemas.CommandResponse:
    ...
    return schemas.CommandResponse(
        room_data=context.current_room_schema, message_to_player=message
    )
