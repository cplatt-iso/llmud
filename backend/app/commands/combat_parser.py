...
from .utils import format_room_mobs_for_player_message, resolve_mob_target, roll_dice
...
async def handle_attack(context: CommandContext) -> schemas.CommandResponse:
    ...
    mobs_in_room_orm = crud.crud_mob.get_mobs_in_room(
    ...
    target_mob_instance, error_or_prompt = resolve_mob_target(
        target_ref_input, mobs_in_room_orm
    )
    ...
    assert (
        target_mob_instance is not None
    )  # Pylance helper if resolver guarantees instance or error message
    ...
    player_damage_dice_str = "1d6"
    player_damage_bonus = 1
    player_ac_temp = 12
    mob_ac = mob_template.base_defense if mob_template.base_defense is not None else 10

    message_parts.append(
        f"<span class='char-name'>{player_display_name}</span> attacks the <span class='inv-item-name'>{mob_template.name}</span>! (HTTP Round)"
    )
    ...
    if (to_hit_roll + player_attack_bonus) >= mob_ac:
        ...
        damage_span_class = (
            "combat-crit" if damage_to_mob > 5 else "combat-hit"
        )  # Example crit style
        message_parts.append(
            f"  <span class='combat-success'>HIT!</span> You deal <span class='{damage_span_class}'>{damage_to_mob}</span> damage. (Roll: {to_hit_roll}+{player_attack_bonus} vs AC {mob_ac})"
        )

        updated_mob_instance = crud.crud_mob.update_mob_instance_health(
            context.db, target_mob_instance.id, -damage_to_mob
        )
        if updated_mob_instance:
            target_mob_instance = updated_mob_instance  # Refresh instance

        if target_mob_instance.current_health <= 0:
            message_parts.append(
                f"  <span class='combat-death'>The {mob_template.name} collapses and DIES!</span>"
            )
            crud.crud_mob.despawn_mob_from_room(
                context.db, room_mob_instance_id=target_mob_instance.id
            )
            combat_ended_in_this_http_round = True
        else:
            message_parts.append(
                f"  The {mob_template.name} has <span class='combat-hp'>{target_mob_instance.current_health}/{mob_template.base_health}</span> HP remaining."
            )
    else:
        message_parts.append(
            f"  <span class='combat-miss'>MISS!</span> (Roll: {to_hit_roll}+{player_attack_bonus} vs AC {mob_ac})"
        )
    ...
    if not combat_ended_in_this_http_round and target_mob_instance.current_health > 0:
        ...
        if (mob_to_hit_roll + mob_attack_bonus) >= player_ac_temp:
            ...
            damage_span_class_mob = (
                "combat-crit-player" if damage_to_player > 4 else "combat-hit-player"
            )
            message_parts.append(
                f"  <span class='combat-success'>HIT!</span> The mob deals <span class='{damage_span_class_mob}'>{damage_to_player}</span> damage to you."
            )
    ...
    if combat_ended_in_this_http_round and target_mob_instance.current_health <= 0:
        remaining_mobs_orm = crud.crud_mob.get_mobs_in_room(
            context.db, room_id=context.current_room_orm.id
        )
        mobs_text, _ = format_room_mobs_for_player_message(
            remaining_mobs_orm, context.active_character
        )
        if mobs_text:
            message_parts.append(mobs_text)
