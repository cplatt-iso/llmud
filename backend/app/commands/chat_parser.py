...
async def handle_chat_command(context: CommandContext) -> schemas.CommandResponse:
    ...
    if channel.access_type == 'permissioned': # Changed from channel['access_type']
        required_perm = channel.required_permission # Changed from channel.get('required_permission')
        has_perm = required_perm and hasattr(character.owner, required_perm) and getattr(character.owner, required_perm)
        if not has_perm:
            return schemas.CommandResponse(message_to_player="You don't have permission to use that channel.")

    # --- Message Construction ---
    if not args:
        return schemas.CommandResponse(message_to_player=f"What do you want to {command}?")
    
    # --- Branch for different channel types ---
    if channel.access_type in ['public', 'permissioned']: # Changed from channel['access_type']
        return await _handle_broadcast_channel(character, channel, args)
        
    elif channel.access_type == 'private' and channel.channel_id_tag == 'tells': # Changed from channel['access_type'] and channel['channel_id_tag']
        return await _handle_tell_channel(db, character, channel, args)

    return schemas.CommandResponse(message_to_player="That channel type is not yet implemented.")


async def _handle_broadcast_channel(character: models.Character, channel: schemas.ChatChannel, args: List[str]) -> schemas.CommandResponse: # Type hint for channel
    ...
    if other_player_ids:
        await connection_manager.broadcast_to_players(
            {"type": "chat_message", "payload": payload_other},
            other_player_ids
        )
    
    # Send confirmation to self
    await connection_manager.send_personal_message(
        {"type": "chat_message", "payload": payload_self},
        sender_player_id
    )

    return schemas.CommandResponse() # No direct message_to_player needed, it's handled via WS


async def _handle_tell_channel(db: Session, character: models.Character, channel: schemas.ChatChannel, args: List[str]) -> schemas.CommandResponse: # Type hint for channel
    if len(args) < 2:
    ...
    if not target_char or not connection_manager.is_player_connected(target_char.player_id):
        return schemas.CommandResponse(message_to_player=f"You can't seem to find '{target_name}' online.")

    if target_char.id == character.id:
        return schemas.CommandResponse(message_to_player="Talking to yourself is the first sign of madness.")
    ...
    # Send the tell
    await connection_manager.send_personal_message(
        {"type": "chat_message", "payload": payload_recipient},
        target_char.player_id
    )
    
    # Send the echo back to the sender
    await connection_manager.send_personal_message(
        {"type": "chat_message", "payload": payload_sender},
        character.player_id
    )
