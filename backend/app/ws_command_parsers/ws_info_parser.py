...
async def handle_ws_look(
    ...
    ground_items_data = [schemas.item.RoomItemInstanceInDB.from_orm(item).model_dump() for item in items_on_ground_orm]
    ...
    look_payload = {
        ...
        "room_data": schemas.RoomInDB.from_orm(current_room_orm).model_dump(exclude_none=True)
    }
