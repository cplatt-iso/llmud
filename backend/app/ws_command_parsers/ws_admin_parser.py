...
async def handle_ws_giveme(
    ...
    if not search_results:
        await combat.send_combat_log(player.id, [f"No item found matching '{args_str}'."])
        return

    if len(search_results) > 1:
        matched_names = ", ".join([item.name for item in search_results])
        await combat.send_combat_log(player.id, [f"Ambiguous query. Multiple items match: {matched_names}"])
        return
    ...
    if _inv_entry:
        await combat.send_combat_log(player.id, [f"You conjure a '{item_to_give.name}' into your inventory."])
...
async def handle_ws_set_god(
    ...
    if len(parts) != 2:
        await combat.send_combat_log(player.id, ["Usage: setgod <character_name> <level>"])
        return
    ...
    if not target_char:
        await combat.send_combat_log(player.id, [f"Character '{target_name}' not found."])
        return
    ...
    await combat.send_combat_log(player.id, [f"You have set {target_char.name}'s god level to {level}."])
