...
from app import crud, models, schemas, websocket_manager
from app.commands.command_args import CommandContext

# --- STEP 1: Import the GOLD STANDARD look handler and its context ---
from app.commands.movement_parser import (
    handle_look,
)
from app.commands.movement_parser import (
    handle_move as http_handle_move,  # Import the HTTP move handler
)
...
async def attempt_player_move(
    ...
    if command_verb == "go":
        if command_args_str:
            ...
            await combat.send_combat_log(player.id, ["Go where?"])
            return
    else:
        raw_direction_input = command_verb.lower()

    target_direction_canonical = direction_map.get(
        raw_direction_input, raw_direction_input
    )

    if (
        not target_direction_canonical
        or target_direction_canonical not in direction_map.values()
    ):
        await combat.send_combat_log(player.id, ["That's not a valid direction."])
        return

    current_room_orm_before_move = crud.crud_room.get_room_by_id(
        db, room_id=character_state.current_room_id
    )
    if not current_room_orm_before_move:
        logger.error(
            f"Character {character_state.name} is in an invalid room ID {character_state.current_room_id}"
        )
        await combat.send_combat_log(
            player.id, ["Error: Your current location is undefined."]
        )
        return
    ...
    if not exit_data_dict:
        ...
        await combat.send_combat_log(player.id, ["You can't go that way."])
        return

    try:
        exit_detail_model = ExitDetail(**exit_data_dict)
        # target_room_orm_for_move is assigned here, but could fail or be None
        target_room_orm_for_move = crud.crud_room.get_room_by_id(
            db, room_id=exit_detail_model.target_room_id
        )

        if not target_room_orm_for_move:
            message_to_player_on_fail = (
                "The path ahead seems to vanish into nothingness."
            )
            ...
            if exit_detail_model.key_item_tag_opens:
                has_key = crud.crud_character_inventory.character_has_item_with_tag(
                    db, character_state.id, exit_detail_model.key_item_tag_opens
                )

            if has_key:
                logger.info(
                    f"Character {character_state.name} unlocking door with key."
                )
                # Unlock logic...
                ...
                await combat.send_combat_log(
                    player.id,
                    [
                        "<span class='system-message-inline'>You unlock the way forward.</span>"
                    ],
                    transient=True,
                )
                moved_successfully = True
                ...
                message_to_player_on_fail = (
                    exit_detail_model.description_when_locked or "That way is locked."
                )
    except Exception as e:
        logger.error(
            f"Error processing movement for {character_state.name}: {e}", exc_info=True
        )
    ...
    if moved_successfully and target_room_orm_for_move:
        old_room_id_for_broadcast = (
            character_state.current_room_id
        )  # Capture before DB update
        ...
        crud.crud_character.update_character_room(
            db, character_id=character_state.id, new_room_id=target_room_orm_for_move.id
        )
        websocket_manager.connection_manager.update_character_location(
            character_state.id, target_room_orm_for_move.id
        )
        ...
        await combat.broadcast_to_room_participants(
            db, old_room_id_for_broadcast, leave_msg, exclude_player_id=player.id
        )
        ...
        await combat.broadcast_to_room_participants(
            db, target_room_orm_for_move.id, arrive_msg, exclude_player_id=player.id
        )
        ...
        look_command_response = await handle_look(
            new_room_context
        )  # Call the imported handle_look

        if look_command_response.special_payload:
            await websocket_manager.connection_manager.send_personal_message(
                look_command_response.special_payload, player.id
            )
...
async def handle_ws_movement(
    ...
    if current_char_state.id in combat.active_combats:
        await combat.send_combat_log(
            player.id,
            ["You cannot move while in combat! Try 'flee'."],
...
async def handle_ws_flee(
    ...
    if current_char_state.id in combat.active_combats and combat.active_combats.get(
        ...
        if flee_direction_arg != "random":
            canonical_flee_dir = direction_map.get(
                flee_direction_arg, flee_direction_arg
            )
            if canonical_flee_dir not in direction_map.values():
                await combat.send_combat_log(
                    player.id,
                    [f"Invalid flee direction '{flee_direction_arg}'."],
                    room_data=current_room_schema,
                    transient=True,
                )
                return
        combat.character_queued_actions[current_char_state.id] = (
            f"flee {canonical_flee_dir}"
        )
        await combat.send_combat_log(
            player.id,
            [
                f"You prepare to flee {canonical_flee_dir if canonical_flee_dir != 'random' else '...'}"
            ],
        )
    else:
        await combat.send_combat_log(
